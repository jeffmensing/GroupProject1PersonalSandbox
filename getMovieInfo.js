//You can run this code using the html file. This file just contains the js
    ...
    function getMoviePlot(movieTitle) {
        ...
        wtf_wikipedia.from_api(queryTitle, "en", function(markup) {
            ...
            for (i = 0; i < object.text.get("Plot").length; i++) {
                ...
                //    document.body.innerHTML = document.body.innerHTML + JSON.stringify(object.text.get("Plot")[i].text);
                moviePlot = moviePlot + JSON.stringify(object.text.get("Plot")[i].text);
            }
            document.body.innerHTML = movieTitle + "<br>" + "<img src='" + movieImg + "'>" + movieYear + "<br>" + moviePlot;
        });
    }

    var movieYear = randomDate("1979", "2017");
    getMovieDetails(movieYear);
});
